...
class CourseAdmin(admin.ModelAdmin):
    list_display = ('title', 'description', 'preview','owner')  # Поля, которые будут отображаться в списке
    search_fields = ('title', 'description')  # Поля, по которым можно будет искать

# Регистрация модели Lesson в админке
@admin.register(Lesson)
class LessonAdmin(admin.ModelAdmin):
    list_display = ('title', 'course', 'description', 'preview', 'owner')  # Поля, которые будут отображаться в списке
